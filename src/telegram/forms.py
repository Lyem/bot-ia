...
form = {
    "create": [
        ...
        {'key': 'cell', 'question': 'Qual é o seu numero de telefone',
            'validate': r"^\(?(?:[14689][1-9]|2[12478]|3[1234578]|5[1345]|7[134579])\)? ?(?:[2-8]|9[1-9])[0-9]{3}\-?[0-9]{4}$"},
        {'key': 'cep', 'question': 'Qual é o seu cep?',
            'validate': r"(\d){5}(\d){3}"},
        {'key': 'home_number', 'question': 'Qual é o numero da sua casa?',
            'validate': r"^[0-9]*$"},
        {'key': 'cpf', 'question': 'Qual é o seu cpf?',
            'validate': r"([0-9]{2}[\.]?[0-9]{3}[\.]?[0-9]{3}[\/]?[0-9]{4}[-]?[0-9]{2})|([0-9]{3}[\.]?[0-9]{3}[\.]?[0-9]{3}[-]?[0-9]{2})"},
    ],
...
def result_form(bot, message):
    if (int(user_data[str(message.chat.id)]['route']) == 0):
        button = Buttons(bot)
        button.plans(message)
    elif (int(user_data[str(message.chat.id)]['route']) == 1):
        pass
    elif (int(user_data[str(message.chat.id)]['route']) == 2):
        menus = Menus(bot)
...
def save_answer(bot, message):
    ...
    if (form[user_data[str(message.chat.id)]['type']][step]['validate'] == None):
        ...
        pat = re.compile(
            form[user_data[str(message.chat.id)]['type']][step]['validate'])
    ...
    if user_data[str(message.chat.id)]['step'] < len(form[user_data[str(message.chat.id)]['type']]):
        ...
        if (user_data[str(message.chat.id)]['type'] == 'create'):
            ...
            if (ava):
                ...
                bot.send_message(
                    message.chat.id, 'O serviço não está disponivel na sua região ;-;')
            ...
            if (ClientExist().execute(user_form['cpf'], message.chat.id)):
                ...
                cpf = user_form['cpf']
                bot.send_message(
                    message.chat.id, f'Não foi encontrado nenhum cliente com o cpf {cpf}')
                Buttons(bot).helpyou(message.chat.id)
            ...
            if (ava):
                bot.send_message(
                    message.chat.id, 'O serviço está disponivel na sua região!')
                Buttons(bot).helpyou(message.chat.id)
            else:
                bot.send_message(
                    message.chat.id, 'O serviço não está disponivel na sua região ;-;')
