...
class Menus:
    ...
    def send_main_menu(self, message):
        ...
        self.bot.send_message(
            chat_id=message.chat.id, text='Selecione uma opção:', reply_markup=menu_markup)
    ...
    def process_choice(self, message) -> bool:
        if message.text == 'Planos':
            ...
            if (ClientLogged().execute(message.chat.id)):
                self.bot.send_message(
                    chat_id=message.chat.id, text='Verificando boletos', reply_markup=types.ReplyKeyboardRemove())
                payments = GetAllByClient().execute(message.chat.id)
                if (len(payments) < 1):
                    self.bot.send_message(
                        chat_id=message.chat.id, text='Você não tem pagamentos pendentes')
                for pay in payments:
                    if (pay.is_pay == False):
                        self.bot.send_message(
                            chat_id=message.chat.id, text=f'boleto do plano {pay.name}')
                        pdf = Generate_pdf().execute(str(message.chat.id), pay.name.replace('Mb', ''))
                        self.bot.send_document(
                            chat_id=message.chat.id, document=open(pdf, 'rb'))
                        os.remove(pdf)
